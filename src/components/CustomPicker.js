          ...
          anchor={
            <View style={{ borderColor: colors.dark, borderWidth: 1, borderRadius: 4, marginVertical: 5 }}>
              <TouchableOpacity onPress={this.props.menuOnPress} style={{ display: "flex", flexDirection: "row", alignItems: "center", justifyContent: "flex-end" }}>
                <Text style={{ fontSize: 17, color: colors.dark, fontWeight: "normal", margin: 15, marginVertical: 17, marginRight: "auto" }}>{(() => {
                  if (this.props.unit) {
                    return this.capitalize(this.props.unit);
                  } else if (this.props.language) {
                    return this.capitalize(this.props.language.name);
                  }
                })()}</Text>
